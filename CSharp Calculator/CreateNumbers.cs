namespace CSharp_Calculator
{
    internal class CreateNumbers
    {
        ...
        internal List<string> CreateNumberGroups()
        {
            ...
            {
                if (StringIsNumber(inputEquation[i]) || (appended == false && (inputEquation[i] == "+" || inputEquation[i] == "-") && i != inputEquation.Count && StringIsNumber(inputEquation[i+1])) || (i != 0 && inputEquation[i] == "." && StringIsNumber(inputEquation[i - 1])))
                {
